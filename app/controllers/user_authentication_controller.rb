class UserAuthenticationController < ApplicationController
  ...
  def index
    ...
    @list_of_users = matching_users.order({ :username => :desc })
      if @current_user != nil
        @list_of_follow_request = FollowRequest.where(:sender_id => @current_user.id)
      end 
  ...
  def show
    ...
    matching_username = User.where({ :username => the_username })

    @the_user = matching_username.at(0)

    @list_of_followers = FollowRequest.where({:recipient_id => @the_user.id, :status => "accepted"})
    @list_of_following = FollowRequest.where({:sender_id => @the_user.id, :status => "accepted"})
    @list_of_photos = Photo.where(:owner_id => @the_user.id)
    @list_of_requests = FollowRequest.where({:recipient_id => @the_user.id, :status => "pending"})
  ...
  def liked_photos
      ...
      @list_of_following = FollowRequest.where({:sender_id => @the_user.id, :status => "accepted"})
      @count = 0
      @list_of_likes = Like.where(:fan_id => @the_user.id)
